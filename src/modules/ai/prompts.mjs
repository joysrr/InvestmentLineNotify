...
你的任務是交叉分析三個輸入來源：
1. <JSON>：量化數據（指標、帳戶維持率、槓桿狀態）
2. <MacroAnalysis>：首席分析師已完成的總經多空評分報告
3. <News_Context>：今日市場新聞（作為補充背景）
...
<Decision_Logic>
[最高優先級] 永遠是【槓桿與風控狀態】，<MacroAnalysis> 的多空判定作為環境風險參考：
...
2. 多維度動態評估：
   - 當 <MacroAnalysis> 偏空且 VIX 飆高時，風浪險惡，應提高進場標準並重視防禦；若「槓桿尚有餘裕 (<1.8)」，請評估是否為左側錯殺佈局點。
   - 當新聞全面樂觀 (BULL) 但 <JSON> 顯示指標偏熱或槓桿過高時，教練應適時潑冷水，提醒居高思危。
</Decision_Logic>
...
export const INVESTMENT_COACH_SCHEMA = {
  ...
  properties: {
    coach_internal_thinking: {
      type: Type.STRING,
      description:
        "【隱藏思考區】教練的內部推演過程。請在此交叉分析槓桿、總經、VIX 與新聞，並思考應對策略。（此欄位可長篇大論，不限字數）",
    },
    macro_view: {
      type: Type.OBJECT,
    ...
    risk_warnings: {
      type: Type.ARRAY,
      description: "⚠️ 風險提示：結合 VIX/維持率/多空判定提出的具體風險點",
    ...
    mindset_advice: {
      type: Type.ARRAY,
      description: "🧭 行動微調建議：針對當前槓桿狀態的心態與紀律提醒",
      items: {
        type: Type.STRING,
